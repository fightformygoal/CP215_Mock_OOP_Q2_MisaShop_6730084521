...
public class MisaShop {
	...
	public static void addItemToOrder(Order order, int index, int amount) {
		...
		// Idea : get index from IndexList then add order to OrderList (which name is "order")
		Item itemIndexed = itemList.get(index) ;
		order.addItem( itemIndexed , amount ) ;  // see addItem method
	}
